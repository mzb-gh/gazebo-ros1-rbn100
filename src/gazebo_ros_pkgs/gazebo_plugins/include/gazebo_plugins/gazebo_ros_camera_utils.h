...
namespace gazebo
{
  ...
  class GazeboRosCameraUtils
  {
    ...
    // global control of sensor active
    protected: int sensor_connect_count_;
    protected: std::shared_ptr<std::mutex> sensor_connect_count_lock_;
    
    /// \brief Keep track of number of image connections
    protected: boost::shared_ptr<int> image_connect_count_;   // 默认空指针，make_shared会初始化
