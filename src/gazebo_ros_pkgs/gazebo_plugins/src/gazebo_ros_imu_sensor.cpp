...
void gazebo::GazeboRosImuSensor::UpdateChild(const gazebo::common::UpdateInfo &/*_info*/)
{
  ...
  if(update_rate>0 && (current_time-last_time).Double() < updata_period) //update rate check
    return;
...
bool gazebo::GazeboRosImuSensor::LoadParameters()
{
  ...
  {
    std::string parent_name = sensor->ParentName();
    std::size_t it = parent_name.find("::");

    robot_namespace = "/" +parent_name.substr(0,it)+"/";
