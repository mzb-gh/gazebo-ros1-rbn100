...
{
  // 全局变量和静态变量不初始化会默认0，但局部的会是随机值
  // 需要有明确初始状态的成员变量需要初始化，其他被赋值的不必
  ...
  this->sensor_connect_count_ = 0;
  this->sensor_connect_count_lock_ = std::shared_ptr<std::mutex>(new std::mutex);
...
void GazeboRosCameraUtils::Load(sensors::SensorPtr _parent,
  ...
  if (this->sdf->HasElement("cameraInfoTopicName"))
    this->camera_info_topic_name_ = this->sdf->Get<std::string>("cameraInfoTopicName");
...
/*   if (!this->image_connect_count_) this->image_connect_count_ = boost::shared_ptr<int>(new int(0));
  if (!this->image_connect_count_lock_) this->image_connect_count_lock_ = 
  ...
  // 对于可运行线程对象，既不join又不detatch，则处于joinable状态，在线程对象析构时程序被终止。
  this->deferred_load_thread_ = boost::thread(boost::bind(&GazeboRosCameraUtils::LoadThread, this));
...
/*   if (!this->camera_name_.empty())
  ...
  /*   camera info publish rate will be synchronized to image sensor publish rates.
    ...
    of image_pipeline. */
/*   ros::AdvertiseOptions cio = ros::AdvertiseOptions::create<sensor_msgs::CameraInfo>(
  ...
  this->camera_info_pub_ = this->rosnode_->advertise<sensor_msgs::CameraInfo>(this->camera_info_topic_name_, 2);
...
/*   if (this->CanTriggerCamera())
  {
    ros::SubscribeOptions trigger_so = ros::SubscribeOptions::create<std_msgs::Empty>(
...
void GazeboRosCameraUtils::TriggerCameraInternal(const std_msgs::Empty::ConstPtr &dummy)
{
  TriggerCamera();
}
...
void GazeboRosCameraUtils::Init()
{
  ...
  double computed_focal_length = (static_cast<double>(this->width_)) / (2.0 * tan(hfov / 2.0));
...
void GazeboRosCameraUtils::PutCameraData(const unsigned char *_src)
{
  ...
  if ((*this->image_connect_count_) > 0)
  {
    ...
    image_msg_.header.frame_id = this->frame_name_;
    image_msg_.header.stamp.sec = this->sensor_update_time_.sec;
    image_msg_.header.stamp.nsec = this->sensor_update_time_.nsec;
    ...
    fillImage(image_msg_, 
              ...
              reinterpret_cast<const void*>(_src));

    this->image_pub_.publish(image_msg_);
  }else{
...
void GazeboRosCameraUtils::PublishCameraInfo()
{
  ...
  // camera info don't need sensor's data
