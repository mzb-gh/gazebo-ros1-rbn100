...
#include <gazebo/msgs/contact.pb.h>
...
void GazeboRosBumper::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  // resolve tf prefix
  std::string prefix;
  this->rosnode_->getParam("tf_prefix", prefix);
  this->frame_name_ = tf::resolve(prefix, this->frame_name_);
  this->contact_pub_ = this->rosnode_->advertise<gazebo_msgs::ContactsState>(this->bumper_topic_name_, 1);
  ...
  this->update_connection_ = this->parentSensor->ConnectUpdated(boost::bind(&GazeboRosBumper::OnContact, this));
...
// 一次碰撞有很多contact，每个contact有很多position，每个position包括碰撞元素，我不明白，
// 但看代码是这个意思，然后整理进ros的消息中。
void GazeboRosBumper::OnContact()
{
  ...
  // this->contact_state_msg_.header.stamp = ros::Time(contacts.time().sec(),contacts.time().nsec());
  this->contact_state_msg_.header.stamp = ros::Time::now();
  ...
  */
  ...
  for (unsigned int i = 0; i < contactsPacketSize; ++i)
  {
    ...
    for (unsigned int j = 0; j < contactGroupSize; ++j)
    {
      ...
      // 动态欧拉角（绕自身固定轴旋转）死锁问题：如果第二步绕Y轴旋转90度，无论第一步和第三步旋转多少度，
      // 最终的X轴始终垂直桌面,Y轴始终平行桌面.
...
{
  // static: 1。改变变量存储区域为静态区（对于函数）；2。改变变量为内部连接（对于全局变量），反义词extern。
  // const：替代预编译指令，因其只是简单的值替换无类型检查。
