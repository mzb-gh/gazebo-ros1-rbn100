...
void MultiCameraPlugin::Load(sensors::SensorPtr _sensor, sdf::ElementPtr /*_sdf*/)
{
  ...
  this->parentSensor = dynamic_pointer_cast<sensors::MultiCameraSensor>(_sensor);
  ...
  for (unsigned int i = 0; i < this->parentSensor->CameraCount(); ++i)
  {
    ...
    // ROS_INFO_STREAM("camera(" << i << ") name [" << cameraName << "]\n");
    ...
    if (cameraName.find("left") != std::string::npos)
    {
      this->newFrameConnection.push_back(this->camera[i]->ConnectNewImageFrame(
          std::bind(&MultiCameraPlugin::OnNewFrameLeft, this, std::placeholders::_1,
                                                              std::placeholders::_2,
                                                              std::placeholders::_3,
                                                              std::placeholders::_4,
                                                              std::placeholders::_5)));
    ...
    {
      this->newFrameConnection.push_back(this->camera[i]->ConnectNewImageFrame(
          std::bind(&MultiCameraPlugin::OnNewFrameRight,this, std::placeholders::_1,
                                                              std::placeholders::_2,
                                                              std::placeholders::_3,
                                                              std::placeholders::_4,
                                                              std::placeholders::_5)));
  ...
  this->update_connection_ = event::Events::ConnectWorldUpdateEnd(
    std::bind(&MultiCameraPlugin::UpdateChild, this));
...
void MultiCameraPlugin::UpdateChild(){}
